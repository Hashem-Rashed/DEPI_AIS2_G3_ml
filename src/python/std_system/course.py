...
class Course:
    ...
    def __init__(self,name):
        self.course_id =course = Course._id_counter
        Course._id_counter +=1
        self.name =name
        self.enrolled_students =[]
    def __str__(self):
        return f"Course ID : {self.course_id}, Nsme : {self.name} , Enrolled: {len(self.enrolled_students)}"
    

    def __repr__ (self):
        return f"Course ID : {self.course_id}, Nsme : {self.name} "
